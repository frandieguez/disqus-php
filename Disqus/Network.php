...
class Network
{
    ...
    public static function dsq_get_query_string($postdata)
    {
        $postdata_str = '';
    ...
    public static function dsq_get_post_content($boundary, $postdata, $file_name, $file_field)
    {
        if (empty($file_name) || empty($file_field)) {
            return self::dsq_get_query_string($postdata);
        }
    ...
    public static function dsq_get_http_headers_for_request($boundary, $content, $file_name, $file_field)
    {
        $headers = array();
    ...
    public static function _dsq_curl_urlopen($url, $postdata, &$response, $file_name, $file_field)
    {
        $c = curl_init($url);
        $postdata_str = self::dsq_get_query_string($postdata);
    ...
    public static function _dsq_fsockopen_urlopen($url, $postdata, &$response, $file_name, $file_field)
    {
        $buf = '';
        $req = '';
        $length = 0;
        $boundary = '----------' . md5(time());
        $postdata_str = self::dsq_get_post_content($boundary, $postdata, $file_name, $file_field);
        ...
        $fp = @fsockopen($url_pieces['host'], $url_pieces['port'], $errno, $errstr, self::$socketTimeout);
        if (!$fp) {
            return false;
        }
        ...
        $req .= 'Host: ' . $host . "\r\n";
        $req .=  self::dsq_get_http_headers_for_request($boundary, $postdata_str, $file_name, $file_field);
        if ($postdata_str) {
            $req .= "\r\n\r\n" . $postdata_str;
        }
        $req .= "\r\n\r\n";
    ...
    public static function _dsq_fopen_urlopen($url, $postdata, &$response, $file_name, $file_field)
    {
        $params = array();
        if ($file_name && $file_field) {
            $boundary = '----------' . md5(time());
            $content = self::dsq_get_post_content($boundary, $postdata, $file_name, $file_field);
            $header = self::dsq_get_http_headers_for_request($boundary, $content, $file_name, $file_field);
            ...
            if ($postdata) {
                $params = array(
                    'http' => array(
                        ...
                        'content'    => self::dsq_get_query_string($postdata),
                        'timeout'    => self::$socketTimeout
                    )
                );
            }
        ...
        ini_set('user_agent', self::getUserAgent());
     ...
     */
    public static function dsq_urlopen($url, $postdata=false, $file=false)
    {
        $response = array(
            'data' => '',
            'code' => 0
        ...
        if (function_exists('curl_init')) {
         if (!function_exists('curl_setopt_array')) {
            ...
            self::dsq_curl_urlopen($url, $postdata, $response, $file_name, $file_field);
        ...
        //     _dsq_fopen_urlopen($url, $postdata, $response, $file_name, $file_field);
            ...
            self::_dsq_fsockopen_urlopen($url, $postdata, $response, $file_name, $file_field);
        }
    ...
    public static function dsq_url_method()
    {
        if (function_exists('curl_init')) {
            return 'curl';
        } elseif (ini_get('allow_url_fopen') && function_exists('stream_get_contents')) {
    ...
    {
        return 'disqus-php/'.DisqusApi::$version;
    }
}
